...
when "production"
  ...
  n = 2
  while n < 110 do
    User.create!(name: xlsx.sheet(0).row(n)[0], party: xlsx.sheet(0).row(n)[1], district: District.find_by(name: xlsx.sheet(0).row(n)[2]), password: xlsx.sheet(0).row(n)[3], password_confirmation: xlsx.sheet(0).row(n)[4], small_district: xlsx.sheet(0).row(n)[5], policy: xlsx.sheet(0).row(n)[6], image: Rails.root.join("public/seoul/#{n}.gif").open, link1: xlsx.sheet(0).row(n)[7], link2: xlsx.sheet(0).row(n)[8], link3: xlsx.sheet(0).row(n)[9], link4: xlsx.sheet(0).row(n)[10], head1: xlsx.sheet(0).row(n)[11], head2: xlsx.sheet(0).row(n)[12], head3: xlsx.sheet(0).row(n)[13], head4: xlsx.sheet(0).row(n)[14])
    n += 1
  end
end
